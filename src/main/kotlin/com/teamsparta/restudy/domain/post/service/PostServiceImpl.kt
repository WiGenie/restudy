...
//    }
    ...
    override fun getPaginatedPostList(pageable: Pageable, postStatus: PostStatus): Page<PostListResponse>? {
//        val postStatus = when (status){
//            "POSTED" -> PostStatus.POSTED
//            "DELETED" -> PostStatus.DELETED
//            else -> throw IllegalArgumentException("status is invalid")
//        }
        return postRepository.findByPageableAndStatus(pageable, postStatus)
    }
    ...
    override fun searchPostList(title: String): List<PostListResponse> {
        return postRepository.searchPostListByTitle(title).map {
            PostListResponse(
                id = it.id!!,
                title = it.title,
                nickname = it.member.nickname,
